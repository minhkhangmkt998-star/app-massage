...
function StatStrip({ bookings, filter, onFilter }) {
  ...
  return (
    <div className="statstrip">
      {tiles.map((t) => (
        <div key={t.key} className={"stat-tile" + (filter === t.key ? " stat-tile--on" : "")} onClick={() => onFilter(filter === t.key ? "all" : t.key)}>
          ...
          <div className="stat-tile__body">
            <div className="stat-tile__l">{window.tr(t.l)}</div>
...
function QueueColumn({ bookings, statFilter, onStatFilter, chipFilter, onChipFilter, selectedId, onSelect, autopilot }) {
  ...
  if (statFilter === "active") list = applyKey("active");
  else if (statFilter === "exception") list = applyKey("exception");
  else if (statFilter !== "all") list = applyKey(statFilter);
  else list = applyKey(chipFilter);
  ...
  return (
    <div className="panel col-queue">
      <div className="panel__head">
        ...
        <h3>{window.tr("Hàng đợi điều phối")}</h3>
        <span className="panel__count">{list.length} {window.tr("đơn")}</span>
      </div>
      <div className="filterbar">
        {chips.map((c) => (
          <button key={c.key} className={"fchip" + ((statFilter === "all" ? chipFilter : null) === c.key ? " fchip--on" : "")}
            ...
            {window.tr(c.vi)} <span className="fchip__n">{countFor(c.key)}</span>
          </button>
        ))}
      </div>
      <div className="queue-scroll">
        {list.length === 0 ? (
          <div className="empty-hint"><I n="inbox" s={18} /> {window.tr("Không có đơn nào")}</div>
        ) : list.map((b) => (
