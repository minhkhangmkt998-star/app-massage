...
function Topbar({ title, sub, region, onRegion, onlineKtv, clock, autopilot, onToggleAuto, lang, onLang }) {
  ...
  return (
    <header className="topbar">
      ...
      <div className="langtog" role="group" aria-label="Language">
        <button className={"langtog__b" + (lang === "vi" ? " langtog__b--on" : "")} onClick={() => onLang("vi")}>VI</button>
        <button className={"langtog__b" + (lang === "en" ? " langtog__b--on" : "")} onClick={() => onLang("en")}>EN</button>
      </div>

      <div className="livechip" title={onlineKtv + " " + tr("KTV đang online")}><span className="livechip__dot"></span><I n="users" s={15} /> {onlineKtv}</div>
          ...
          <div style={{ position: "absolute", top: 46, right: 0, zIndex: 30, background: "#fff", border: "1px solid var(--color-border)", borderRadius: 8, boxShadow: "var(--elevation-2)", padding: 6, width: 180 }}>
            {["Tất cả khu vực", ...REGIONS].map((r) => (
              <button key={r} onClick={() => { onRegion(r); setOpen(false); }}
                style={{ display: "block", width: "100%", textAlign: "left", border: "none", background: r === region ? "var(--teal-50)" : "transparent", color: r === region ? "var(--color-primary)" : "var(--text-primary)", fontWeight: r === region ? 700 : 600, padding: "9px 11px", borderRadius: 8, cursor: "pointer", fontSize: 11.6 }}>
                {tr(r)}
              </button>
            ))}
          </div>
      ...
      <div className="region-sel region-sel--clock" style={{ cursor: "default", gap: 8, color: "var(--text-secondary)" }} title={tr("Giờ hệ thống")}>
        <I n="clock" s={16} style={{ color: "var(--text-secondary)" }} /> <span className="mono" style={{ color: "var(--text-primary)" }}>{clock}</span>
      </div>

      <button className="icon-pill" title={tr("Thông báo")}><I n="bell" s={19} /><span className="icon-pill__count">3</span></button>
    </header>
  );
}
